...
def remove_duplicates(directory):
    ...
    for dir, _, file in os.walk(directory):
        ...
        for filename in files_dict:
            ...
            f.write(f"{datetime.datetime.today()}: File detail {file_detail}")
            latest_file_path = file_detail[-1][1]
            latest_file_extension = file_detail[-1][2]

            for file_number, file_path, _ in file_detail[:-1]:
                f.write(f"{datetime.datetime.today()}: Deleted duplicate file: {file_path}\n")
                os.remove(file_path)

            os.rename(latest_file_path, os.path.join(dir, f"{file_name}{latest_file_extension}"))
            f.write(f"{datetime.datetime.today()}: Rename {latest_file_path} to {file_name}{latest_file_extension}\n")
